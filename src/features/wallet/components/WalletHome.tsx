...
export const WalletHome: React.FC = () => {
  ...
  React.useEffect(() => {
    // Auto-load balances when component mounts
    if (activeAccount) {
      // The useTokens hook handles loading automatically
    }
  }, [activeAccount])
          ...
          <Card className="p-6">
            <div className="text-center">
              <p className="text-sm text-matte-black-600 mb-2">
                {t('wallet.total_balance') || 'Saldo Total Estimado'}
              </p>
              
              {loadingBalances ? (
                <div className="flex items-center justify-center py-4">
                  <LoadingSpinner size="sm" />
                  <span className="ml-2 text-matte-black-600">
                    {t('wallet.loading_balance') || 'Carregando saldo...'}
                  </span>
                </div>
              ) : (
