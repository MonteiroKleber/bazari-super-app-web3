...
type ModalType = 'create' | 'import-seed' | 'import-json' | 'watch' | 'rename' | 'export' | 'remove' | null
...
export const AccountsManager: React.FC = () => {
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      await addFromMnemonic(mnemonic, accountName)
  ...
  const handleImportSeed = async () => {
    ...
    setIsLoading(true)
    try {
      await addFromMnemonic(mnemonicInput, accountName || 'Conta Importada')
  ...
  const handleImportJson = async () => {
    ...
    try {
      ...
      await importJson(jsonData, jsonPassword)
      toast.success(t('wallet.account_imported') || 'Conta importada com sucesso!')
      closeModal()
    } catch (error) {
  ...
  const handleAddWatchOnly = () => {
    ...
    try {
      addWatchOnly(watchAddress, accountName || 'Conta Watch-Only')
  ...
  const handleRename = () => {
    ...
    rename(selectedAccountId, accountName)
  ...
  const handleExport = async () => {
    ...
    setIsLoading(true)
    try {
      const exportData = await exportJson(selectedAccountId, exportPassword)
  ...
  const openModal = (type: ModalType, accountId?: string) => {
    ...
    // Pre-fill data for specific modals
    if (type === 'create') {
    ...
    <div className="min-h-screen bg-sand-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        >
          <Card className="p-6">
            <h2 className="text-lg font-semibold text-matte-black-900 mb-4">
              {t('wallet.accounts.my_accounts') || 'Minhas Contas'} ({accounts.length})
            </h2>
                ...
                <motion.div
                  ...
                  className={`p-4 rounded-xl border transition-all ${
                    account.id === activeAccount?.id
                      ...
                      : 'border-sand-200 bg-white hover:shadow-soft'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Button
                        onClick={() => openModal('rename', account.id)}
                        variant="ghost"
                        size="sm"
                      >
                        <Edit2 size={16} />
                      ...
                      {!isWatchOnly(account) && (
                        <Button
                          onClick={() => openModal('export', account.id)}
                          variant="ghost"
                          size="sm"
                        >
                          <Download size={16} />
      ...
      <AnimatePresence>
        {modalType && (
          <AccountModal
            ...
            selectedAccountName={accounts.find(acc => acc.id === selectedAccountId)?.name || ''}
          />
        )}
      </AnimatePresence>
    </div>
...
const AccountModal: React.FC<AccountModalProps> = (props) => {
  ...
  const getModalConfig = () => {
    switch (props.type) {
      case 'create':
        return {
          title: t('wallet.accounts.create_title') || 'Criar Nova Conta',
          size: 'lg' as const
        }
      case 'import-seed':
        return {
          title: t('wallet.accounts.import_seed_title') || 'Importar com Seed',
          size: 'md' as const
        }
      case 'import-json':
        return {
          title: t('wallet.accounts.import_json_title') || 'Importar JSON',
          size: 'md' as const
        }
      case 'watch':
        return {
          title: t('wallet.accounts.watch_title') || 'Adicionar Watch-Only',
          size: 'md' as const
        }
      case 'rename':
        return {
          title: t('wallet.accounts.rename_title') || 'Renomear Conta',
          size: 'sm' as const
        }
      case 'export':
        return {
          title: t('wallet.accounts.export_title') || 'Exportar Conta',
          size: 'sm' as const
        }
      case 'remove':
        return {
          title: t('wallet.accounts.remove_title') || 'Remover Conta',
          size: 'sm' as const
        }
      default:
        return { title: '', size: 'md' as const }
    }
  }
              ...
              <div className="flex items-start space-x-2">
                ...
                <div>
                  <p className="text-sm font-medium text-bazari-red-800 mb-1">
                    {t('wallet.seed_warning_title') || 'Importante!'}
                  </p>
              ...
              <div className="bg-sand-50 rounded-lg p-4 mb-4">
                {showSeed ? (
                  <div className="grid grid-cols-3 gap-2">
                    {props.seedPhrase.map((word, index) => (
                      <div key={index} className="flex items-center space-x-2">
                        <span className="text-xs text-matte-black-500 w-6">
                          {index + 1}.
                        </span>
